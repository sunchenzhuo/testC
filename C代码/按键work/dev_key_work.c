 ...
 * @LastEditTime: 2025-11-27 10:49:04
...
struct pdev
{
    ...
    struct work_struct work; // 下半部处理机制
};
...
/*
中断函数不做耗时操作，只把真正的按键处理放到工作队列里执行。
消抖 → 判断是否按下 → 更新按键值 → newflag=1 → 唤醒用户空间
*/
void work_queue_func(struct work_struct *pwk)
{
    struct pdev *gkeydev = (container_of(pwk, struct pdev, work)); // 通过 work_struct 的地址，计算出整个 struct pdev 的起始地址。
    int status = 0;
...
/*
中断处理函数 key_handler()
中断触发 → schedule_work() → 立刻返回 → 工作队列处理按键
*/
irqreturn_t key_handler(int nu, void *arg)
{
    struct pdev *gkeydev = (struct pdev *)arg; // arg 是申请中断时传入的 pkeydev
    schedule_work(&gkeydev->work);             // 立即调度工作队列
